...
import {
  Cpu, Database, Play,
  Loader, CheckCircle, ChevronRight,
  BarChart3, Sparkles, Layers, GitMerge,
  Zap, Info, TrendingUp, Award, X
} from "lucide-react";
...
const COLOR_MAP = {
  ...
  cyan: { bg: "bg-cyan-500/20", border: "border-cyan-400/40", text: "text-cyan-400" },
  green: { bg: "bg-green-500/20", border: "border-green-400/40", text: "text-green-400" },
};
...
function MetricCard({ label, value, highlight }) {
  return (
    <div className={`p-4 rounded-2xl text-center border ${highlight ? "bg-purple-500/20 border-purple-400/30" : "bg-white/5 border-white/10"}`}>
      <div className={`text-2xl font-bold ${highlight ? "text-purple-300" : "text-white"}`}>{value}</div>
      <div className="text-xs text-white/40 mt-1 capitalize">{label.replaceAll("_", " ")}</div>
    </div>
  );
}
...
function ModelLab() {
  ...
  const [allDatasets, setAllDatasets] = useState({ builtin: [], imported: [] });
  ...
  const fetchAiSuggestions = async () => {
    ...
    try {
      ...
      if (data.suggestions?.length > 0) {
        setSelectedModels(data.suggestions.map(s => s.model_id));
      }
    } finally {
  ...
  const STEPS = ["Dataset", "Target", "Mode", "Configure", "Results"];

  return (
    <div
      className="min-h-screen text-white"
      style={{ background: "radial-gradient(ellipse at top,#0f0c29,#302b63,#24243e)" }}
    >
        ...
        <div className="flex items-center gap-1 mb-8 overflow-x-auto">
          {STEPS.map((s, i) => (
            <div key={i} className="flex items-center gap-1 flex-shrink-0">
              <div className={`px-3 py-1.5 rounded-xl text-xs font-medium transition ${
                step === i + 1
              ...
              {i < 4 && <ChevronRight size={12} className="text-white/20" />}
            </div>
          ))}
        </div>
        ...
        {step === 4 && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <div className="mb-6">
                <h1 className="text-3xl font-bold mb-2">Configure Experiment</h1>
                <p className="text-white/40 text-sm">
                  {experimentMode === "single" && "Pick one model to train."}
                  {experimentMode === "compare" && "Select 2 or more models to compare side by side."}
                  {experimentMode === "ensemble" && "Select models to combine into one powerful ensemble."}
                  {experimentMode === "optimize" && "Select models and configure optimization techniques."}
                </p>
              </div>
                    ...
                    {aiSuggestions.suggestions.map((s, i) => (
                      <div key={i} className="flex items-start gap-3 text-xs">
                        ...
                        <div>
                          ...
                          <span className="text-white/40 ml-2">— {s.reason}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {models.map((model) => {
                  ...
                  const isSuggested = aiSuggestions?.suggestions?.some(s => s.model_id === model.id);
                  return (
                    <button
                      key={model.id}
            ...
            <div className="space-y-4">
              <div className="p-5 rounded-2xl bg-white/5 border border-white/10">
                <p className="text-sm font-semibold mb-3">Selected ({selectedModels.length})</p>
                {selectedModels.length === 0 ? (
                  <p className="text-xs text-white/30">No models selected yet</p>
                ) : (
                  <div className="space-y-2">
                    {selectedModels.map((id, i) => (
                      <div key={i} className="flex items-center justify-between text-xs">
                        ...
                        <button onClick={() => toggleModel(id)} className="text-white/30 hover:text-red-400">
                          <X size={12} />
                        </button>
                      </div>
                    ))}
                ...
                <div className="p-5 rounded-2xl bg-green-500/5 border border-green-400/20">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-white/30">More reliable evaluation</p>
                    </div>
                    <button
                      onClick={() => setUseCV(!useCV)}
                      className={`w-10 h-5 rounded-full transition relative ${useCV ? "bg-green-500" : "bg-white/20"}`}
                    >
                      <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${useCV ? "left-5" : "left-0.5"}`} />
                    </button>
                  </div>
                  ...
                  {useCV && (
                    <div className="mb-4">
                      ...
                      <input
                        type="range" min="3" max="10" value={cvFolds}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFeatureSelection(e.target.value ? parseInt(e.target.value) : null)}
              ...
              <button
                ...
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-purple-500 to-cyan-500 hover:from-purple-400 hover:to-cyan-400 transition font-bold text-sm disabled:opacity-40 flex items-center justify-center gap-2"
              >
                {training ? (
                  <>
